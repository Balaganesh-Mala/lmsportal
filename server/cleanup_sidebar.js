...
const sidebarStart = '            {/* Sidebar Code (Timeline / Platform Flat UI Style) */}';
const sidebarEnd = '                <div className="overflow-y-auto flex-1 pb-10 m-0 space-y-0';

// I'll use a more robust way to find the sidebar block and the following div
const logicBody = `
                                    let total = 0, comp = 0;
                                    modules.forEach(m => {
                                        (m.topics || []).forEach(t => {
...
// Use a regex to catch the mess from line 673 to where the list starts (line 812)
const messRegex = /<div className=\{\`[\s\S]*?bg-\[#1B2538\] border-r border-\[#152040\][\s\S]*?bg-\[#1B2538\]">/;

content = content.replace(messRegex, cleanSidebarHeader);

fs.writeFileSync(path, content, 'utf8');
console.log('Cleaned up sidebar and fixed syntax error');
