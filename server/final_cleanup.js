...
// Fix redundant overall progress bars in the header
// We want to find the whole header section and replace it with a clean one.
const headerRegex = /\{(\/\* Sidebar Code .*? \*\/)\}\s+<div className=\{\`[\s\S]*?bg-\[#1B2538\] border-r border-\[#152040\][\s\S]*?bg-\[#1B2538\]">/;
const cleanSidebarPart = \`{/* Sidebar Code (Timeline / Platform Flat UI Style) */}
            <div className={\\\`
...
// I'll use a very aggressive search and replace for the entire sidebar area
// Search from "Sidebar Code" to the beginning of the modules loop
const bigMessRegex = /\{\/\* Sidebar Code[\s\S]*?\{modules\.length === 0 && \(/;
const replacement = cleanSidebarPart + '\\n\\n                    {modules.length === 0 && (';

content = content.replace(bigMessRegex, replacement);

fs.writeFileSync(path, content, 'utf8');
console.log('Successfully cleaned up CoursePlayer.jsx');
